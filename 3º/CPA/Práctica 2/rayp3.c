...
void render(const unsigned size, const Sphere spheres[size])
{
  ...
  #pragma omp parallel private(contTrace,n)
  {
    contTrace = 0;
    #pragma omp for private (ncalls_line, x, raydir, pixel, ncalls, xx, yy)
    for (y = 0; y < height; ++y)
    {
    ncalls_line = 0;
      for (x = 0; x < width; ++x) {
    ...
    printf("Thread %d: %d calls to trace\n", n, contTrace);
  } 
  double t2 = omp_get_wtime();
  double time = t2-t1;
  print_histogram(histo);
  printf("Least complex line was %d and it had %6.3f calls to trace per pixel.\n", min_line, min_ncalls_line);
  printf("Most  complex line was %d and it had %6.3f calls to trace per pixel.\n", max_line, max_ncalls_line);
  printf("Tiempo: %f\n", time);
