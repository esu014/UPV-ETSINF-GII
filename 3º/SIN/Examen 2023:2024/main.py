...
#heuristica Piezas Descolocadas
def getDescolocadas(state):
    tot = 0
    for i in range(0,9):
        pos = end_state.index(str(i))
        posX = int(pos/ 3)
        posY = pos%3
        idx = state.index(str(i))
        itemX = int (idx/3)
        itemY = idx%3
        if (abs(posX - itemX) + abs(posY - itemY)) !=0:
            tot +=1
    return tot
    
#Heuristica Secuencia
def getSecuencias(string_state):
    res = 0
    indices = [1, 2, 5, 0, 0, 8, 3, 6, 7]
    for x in range(0, 9):
        if x == 4:
            res = res
        elif string_state[x] == '0' and x != 4:
            res = res + 3
        elif x == 4:
            res = res
        elif string_state[x] == '8' and string_state[indices[x]] == '1':
            res = res
        elif str(int(string_state[x]) + 1) != string_state[indices[x]]:
            res = res + 2
    print(3*res)
    return res * 3

#Heuristica GetMin
def getMin(string_state):
    sec = getSecuencias(string_state)
    desc = getDescolocadas(string_state)
    return min(sec, desc )
