...
int main(int argc, char *argv[])
{
  ...
  if (me == 0) {
    ...
    for (proc = 1; proc < num_procs; proc++) {
      MPI_Send(&M[proc*sz], sz, MPI_DOUBLE, proc, 13, MPI_COMM_WORLD);//->Scatter
      MPI_Send(&v[proc*mb], mb, MPI_DOUBLE, proc, 89, MPI_COMM_WORLD);//->Scatter
      MPI_Send(x, n, MPI_DOUBLE, proc, 25, MPI_COMM_WORLD);//->Bcast
    }
  } else {
...
#define ABS(a) ((a) >= 0 ? (a) : -(a))
  ...
  aux = norma;
  MPI_Reduce(&aux, &norma, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
