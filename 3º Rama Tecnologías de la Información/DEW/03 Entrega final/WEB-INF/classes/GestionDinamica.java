...
public class GestionDinamica extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("opt") == null) {
			opt = "";
		} else {opt = request.getParameter("opt");}
		...
		if(opt.equals("imagen")) {
			...
			{
	            res.put("err", "imagen no encontrada");
	        }
		...
		if(request.getParameter("imagen")!=null)
		{
			...
			response.setContentType("application/json");
			response.setCharacterEncoding("UTF-8");
			String dniUSR = request.getParameter("imagen");
			String carpeta = getServletContext().getInitParameter("Directorio_imagenes");
			String img =null;
			try(BufferedReader br = new BufferedReader(new FileReader(carpeta+"/"+dniUSR+".pngb64")))
			{
				StringBuilder sb = new StringBuilder();
	            String line;
	            while ((line = br.readLine()) != null) {
